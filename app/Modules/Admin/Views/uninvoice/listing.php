...
<div style="padding:22px;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
        <div>
            <h1 style="font-size:22px;font-weight:900;color:#18243c;margin:0;">Unregistered Bill of Supply</h1>
            <p style="color:#718096;font-size:12px;margin:4px 0 0;">UBOS register · firm <?= esc(fy()->firm_name ?? '') ?> · FY <?= esc(fy()->FY ?? '') ?></p>
        </div>
        <a href="<?= site_url('admin/uninvoice/add') ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Add UBOS</a>
    </div>

    <form method="get" action="<?= site_url('admin/uninvoice/listing') ?>" style="background:#fff;border-radius:10px;padding:16px;margin-bottom:16px;box-shadow:0 12px 26px rgba(24,36,60,.06);">
        <div class="row">
            ...
            <div class="col-sm-3" style="display:flex;align-items:flex-end;gap:8px;">
                ...
                <a href="<?= site_url('admin/uninvoice/listing') ?>" class="btn btn-default"><i class="fa fa-refresh"></i> Reset</a>
            </div>
        </div>
    </form>

    <div style="background:#fff;border-radius:10px;padding:16px;box-shadow:0 12px 26px rgba(24,36,60,.06);">
        <table id="ubos-grid" class="table table-striped table-hover" style="width:100%;">
            <thead><tr><th>#</th><th>Bill</th><th>FY</th><th>Date</th><th>Party</th><th>Product</th><th>Qty</th><th>Total</th><th>Status</th></tr></thead>
            <tbody></tbody>
        </table>
    </div>
</div>
...
<script>
    (function () {
        jQuery('#ubos-grid').DataTable({
            processing: true, serverSide: true, order: [],
            ajax: { url: "<?= site_url('admin/uninvoice/view_all') ?>" + window.location.search, type: 'POST' },
            columnDefs: [{ orderable: false, targets: [0, 2, 3, 4, 5, 6, 7, 8] }]
        });
    })();
</script>
