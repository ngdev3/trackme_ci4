...
<main class="main-content"><div id="mainContent"><div class="container-fluid" style="max-width:1480px;margin:0 auto;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <h3 style="font-weight:900;margin:0;">Documents Renewal</h3>
        <a href="<?= base_url('admin/document/add'); ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Add Document</a>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e3e9f2;border-radius:12px;padding:16px;">
        <table class="table table-striped table-bordered" id="employee-grid-buyer" style="width:100%">
            <thead><tr><th>#</th><th>Name</th><th>Remark</th><th>Start</th><th>End</th><th>Updated</th><th>Scope</th><th>File</th><th>Validity</th><th>Action</th></tr></thead>
        </table>
    </div>
</div></div></main>
...
$('#employee-grid-buyer').dataTable({bStateSave:false,processing:true,serverSide:true,responsive:true,pageLength:25,pagingType:"bootstrap_full_number",
 ajax:{url:BASE+"admin/document/view_all?"+QS,type:"post",error:function(){$("#employee-grid_processing").hide();}},order:[]});
$(document).on('click','.doc-del',function(){if(!confirm('Permanently delete this document + its file?'))return;
 $.post(BASE+"admin/document/delete",{id:$(this).data('id')},function(r){if(r&&r.status==='success'){location.reload();}else{alert((r&&r.message)||'Delete failed');}},'json');});
</script>
