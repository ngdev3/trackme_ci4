<?php
/** View-Only Users manager — CI4, theme-aligned. Super-Admin-only. */
helper(['url']);
$users = $users ?? [];
$onCount = 0;
foreach ($users as $u) { if ((int) ($u->is_view_only ?? 0) === 1) { $onCount++; } }
?>
<style>
  .vo{color:var(--tm-ink,#18243c);padding:22px}.vo-shell{margin:0 auto;max-width:1120px}
  ...
  #vo-table{border-collapse:separate;border-spacing:0;width:100%;min-width:720px}
  ...
  .vo-switch{position:relative;display:inline-flex;align-items:center;cursor:pointer;user-select:none;gap:9px;margin:0;font-weight:700;font-size:12px;color:#516174}
  ...
  .vo-selall{font-weight:700;font-size:12.5px;color:#516174;display:inline-flex;align-items:center;gap:8px;cursor:pointer}
...
<main class="main-content bgc-grey-100 vo">
  <div id="mainContent">
    <div class="container-fluid vo-shell">
      ...
      <section class="vo-hero">
        <div class="vo-hero-l">
          ...
          <div>
            ...
            <p>Make a user global read-only — they can view but never add, edit, update or delete.</p>
          </div>
        </div>
      ...
      <div class="vo-note">
        <b>How it works:</b> a view-only user can <b>see</b> every module they already have access to, but
        <b>Add, Edit, Update &amp; Delete are blocked everywhere</b> in the app (this panel and the old one share the
        same users). <b>Super Admin is never restricted</b> and is not listed here.
      </div>
      ...
      <form method="post" action="<?= base_url('admin/setting/save_view_only') ?>" id="view-only-form">
        ...
        <div class="vo-card">
            ...
            <table id="vo-table">
              <thead>
                <tr>
                  ...
                  <th style="width:170px;text-align:center;">View Only</th>
                </tr>
              </thead>
              <tbody>
                <?php if (empty($users)): ?>
                ...
                <?php else: $i = 0; foreach ($users as $u): $i++;
                  ...
                  $on = ((int) ($u->is_view_only ?? 0) === 1);
                  $active = strtolower((string) ($u->status ?? '')) === 'active';
                ?>
                  <tr class="vo-row" data-search="<?= esc(strtolower($name . ' ' . ($u->email ?? '') . ' ' . ($u->mobile ?? ''))) ?>">
                    <td><?= $i ?></td>
                    <td>
                      <div class="vo-uname"><?= esc($name) ?> <?php if ($on): ?><span class="vo-badge vo-badge-on">VIEW ONLY</span><?php endif; ?></div>
                    </td>
                    <td>
                    ...
                    <td style="text-align:center;">
                      <label class="vo-switch">
                        <input type="checkbox" class="vo-check" name="view_only_ids[]" value="<?= (int) $u->id ?>" <?= $on ? 'checked' : '' ?>>
          ...
          <div class="vo-foot">
            <label class="vo-selall"><input type="checkbox" id="vo-all" style="width:16px;height:16px"> Select all (visible)</label>
...
<script>
(function () {
    function updateCount() { jQuery('#vo-count').text(jQuery('.vo-check:checked').length); }
    jQuery('#vo-all').on('change', function () { jQuery('.vo-row:visible .vo-check').prop('checked', this.checked); updateCount(); });
    jQuery(document).on('change', '.vo-check', updateCount);
